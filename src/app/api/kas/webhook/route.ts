...
export async function POST(req: NextRequest) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    const { data: plan } = await db.from('kas_plans').select('credits_per_month').eq('id', plan_id).single()
    if (!plan) return NextResponse.json({ received: true })

    const subId = session.subscription as string
    const stripeSub: Stripe.Subscription = await stripe.subscriptions.retrieve(subId)
  ...
  if (event.type === 'customer.subscription.updated') {
    ...
    await db.from('kas_subscriptions')
      .update({ status: sub.status as 'active' | 'past_due' | 'canceled', updated_at: new Date().toISOString() })
